...
public class ImageThumbnailHandler implements RequestHandler<S3Event, String> {
    ...
    //sam local generate-event s3 put --bucket routing-lambda-bucket --key Rose-PNG-Free-File-Download.png --region ap-south-1 > s3-put-event.json
    //sam local invoke ImageThumbnailFunction --region ap-south-1 --profile serverless-admin -e s3-put-event.json
    //sam local invoke ImageThumbnailFunction --region ap-south-1 --profile serverless-admin -d 6898 -e s3-put-event.json --> to attach the debugger create a remote debug configuration and attach debugger
    @Override
